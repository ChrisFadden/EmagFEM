...
m=3; n=3; % includes boundary nodes, mesh spacing 1/(m-1) and 1/(n-1)
...
K=zeros(N,N); % zero matrix in sparse format: zeros(N) would be "dense"
...
for e=1:T  % integration over one triangular element at a time
  nodes=t(e,:); % row of t = node numbers of the 3 corners of triangle e  Pe=[ones(3,1),p(nodes,:)]; % 3 by 3 matrix with rows=[1 xcorner ycorner] 
...
K(b,b)=eye(length(b),length(b)); % put I into boundary submatrix of K
